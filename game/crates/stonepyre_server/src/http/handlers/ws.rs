...
async fn handle_interaction(
    ...
    match (action, target.clone()) {
        ...
        (InteractionAction::ChopDown, InteractionTarget::Tile(tile)) => {
            ...
            if let Some(loot) = loot_preview {
                match crate::game::sim::inventory::can_character_inventory_accept_item(
                    &state.db,
                    character_id,
                    loot.item_id,
                    loot.quantity,
                ...
                .await
                {
                    Ok(check) if !check.can_accept => {
                        info!(
                            "harvest interaction rejected because inventory is full character_id={} item_id={} quantity={} slots_used={} slots_total={} additional_slots_required={}",
                            character_id,
                            check.item_id,
                            check.quantity,
                            check.slots_used,
                            check.slots_total,
                            check.additional_slots_required
                        );
                    ...
                    Err(e) => {
                        warn!(
                            "game ws inventory capacity precheck failed character_id={} item_id={} quantity={} error={:?}",
                            character_id, loot.item_id, loot.quantity, e
                        );
