...
def resize_cover(img: Image.Image, tw: int, th: int) -> Image.Image:
    ...
    # For “fit-to-template” you used LANCZOS before.
...
def quantize_to_6tone(img: Image.Image) -> Image.Image:
...
def stonepyre_greyscale(
    img: Image.Image,
    *,
    target_size: Optional[Tuple[int, int]] = None,
) -> Image.Image:
    """
    Applies Stonepyre 6-tone quantization. Optionally cover-resizes to target_size first.
    """
    if target_size is not None:
        tw, th = target_size
        img = resize_cover(img.convert("RGBA"), tw, th)
    return quantize_to_6tone(img)

def scale_nearest(img: Image.Image, scale: int) -> Image.Image:
    """
    Pixel-crisp integer upscaling.
    """
    if scale <= 1:
        return img
    w, h = img.size
...
def main():
    ...
    OUTPUT_DIR = Path(r"C:\Users\ryanj\Development\GameProjects\Stonepyre\tools\greyscale_outputs")
    ...
    quantized = stonepyre_greyscale(img, target_size=(TARGET_W, TARGET_H))
